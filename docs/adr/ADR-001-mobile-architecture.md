...
**Phase 1 — Usable daily driver (3–4 weeks).** Password-provider auth (D5 rung 2) with Keychain storage; v2 connection registry through upstream's Settings → Gateway UI; photo/file attach; Share-sheet saves; local notifications for turn completion; power/battery signals; `mobile.css` pass on chat, sidebar, settings, model picker, command palette; edge-swipe rails; TestFlight build; the parity suite (D8) on every PR and weekly against upstream `main`. Enterprise quality requirements carried from the messaging-app audit:

- **Bridge request deduplication.** Upstream fires bursts of identical REST calls on boot and reconnect (`refreshProfiles`, `setup.runtime_check`, config reads). The bridge's `api()` must dedupe in-flight requests with the same `(method, path, body)` key so the gateway sees one call, not three.
- **Bridge callback memoization.** Every `on*` subscription the bridge registers (≈40 callback setters) must return stable unsubscribe handles and must not trigger React re-renders when the same callback is re-registered.
- **Edge-swipe gesture.** Must use velocity threshold + cancel zone (not just distance), with haptic tick on commit. Match iOS system back-swipe feel.
- **Loading / reconnect shimmer.** Gateway disconnect → reconnect must show a skeleton or shimmer state, never a flash of empty content or a stale error. Applies to the connection-lost banner and any mobile-owned overlay.
- **rAF / timer cleanup.** All mobile-owned `requestAnimationFrame`, `setTimeout`, and `setInterval` calls must be cancelled on component unmount or bridge teardown. No leaked timers.

**Phase 2 — Native integration (3–4 weeks, partly gated on the upstream PR).** System-browser OAuth (`ASWebAuthenticationSession`) + Hermes Cloud sign-in behind `auth_flows` detection; `hermes://` deep links; Share Extension ("send to Hermes" text/URL/image into a session); haptics via Capacitor `Haptics` (upstream's `HapticsProvider` uses `web-haptics`, an AudioContext-driven technique that does not reach the Taptic Engine; the bridge registers a native trigger through `@/lib/haptics`' `registerHapticTrigger` seam); voice input through `requestMicrophoneAccess` + the gateway transcription API; theme marketplace fetch; iPad layout (>640 px docks rails natively already). Enterprise quality requirement:
